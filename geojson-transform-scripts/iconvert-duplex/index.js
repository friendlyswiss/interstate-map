  ...
  fs.readFile(myArgs[i], 'utf8' , (err, data) => {
    ...
    for (let j = 0; j < featureCount; j++) {
      ...
      if (props.routes.length == 3){
        ...
        //...make a deep copy with a newFeature variable
        let newFeature1 = JSON.parse(JSON.stringify(data.features[j]))
      
        let m = 1     
        for (let k = 0; k < 2; k++) {
          newFeature1.properties["route" + k + "_ref"] = props.routes[m].name
          newFeature1.properties["route" + k + "_status"] = props.routes[m].status
          newFeature1.properties["route" + k + "_role"] = props.routes[m].role
          newFeature1.properties["route" + k + "_tensDigit"] = props.routes[m].tensDigit
          newFeature1.properties["route" + k + "_onesDigit"] = props.routes[m].onesDigit
          newFeature1.properties["route" + k + "_tier"] = props.routes[m].tier
          m++
        }

        //Add the new feature to the feature array within data
        data.features.push(newFeature1)
   
        //...make a deep copy with a newFeature variable
        newFeature2 = JSON.parse(JSON.stringify(data.features[j]))
        
        m = 0
        for (let k = 0; k < 2; k++) {
          newFeature2.properties["route" + k + "_ref"] = props.routes[m].name
          newFeature2.properties["route" + k + "_status"] = props.routes[m].status
          newFeature2.properties["route" + k + "_role"] = props.routes[m].role
          newFeature2.properties["route" + k + "_tensDigit"] = props.routes[m].tensDigit
          newFeature2.properties["route" + k + "_onesDigit"] = props.routes[m].onesDigit
          newFeature2.properties["route" + k + "_tier"] = props.routes[m].tier
          m++
          m++
        }

        //Add the new feature to the feature array within data
        data.features.push(newFeature2)
